<template>
  <div class="view-head">
    <VueDraggableNext v-model="viewList" @end="onEnd">
      <transition-group class="viwe-list">
        <div class="viwe-list--item" :class="{ active: activeId === item.id }" @click="changeView(item)"
          v-for="item in viewList" :key="item.id">
  ...
  <grid-layout :layout.sync="activeView" :col-num="24" :row-height="10" :margin="[16, 16]">
    <grid-item v-for="it in activeView" :x="it.x" :y="it.y" :w="it.w" :h="it.h" :i="it.i" :static="true" :key="it.i"
      class="griditem">
