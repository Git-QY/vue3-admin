...
const schemaRules = {
  id: { type: Number, default: () => $generateUUID() }, // uuid
  type: {
    type: String,
    enum: ['single_choice', 'multiple_choice', 'fill_in_the_blank', 'true_or_false', 'short_answer', 'essay'],
    required: true,
  }, // 单选、多选、填空、判断、简答、论述/作文等
  stem: { type: String, required: true }, // 题干描述
  tags: [{ type: String, enum: ['1', '2', '3', '4', '5'] }], // 所属知识点标签
...
const interviewValidationRules = () => [
  ...
  body('tags').optional({ checkFalsy: true }).isString().withMessage('知识点标签必须为字符串'),
